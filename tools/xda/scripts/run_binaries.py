...
def main(pkg, arch, comp, pie, opt, timeout, dev_id):
  ...
  for name in os.listdir(bin_dir):
    ...
    stime = time.time()
    os.system('timeout %s python3 /packages/XDA/run_xda.py %s %s %s' % (timeout, bin_path, res_path, dev_id))
    #os.system('timeout %s python3 /packages/XDA/run_xda.py %s %s' % (timeout, bin_path, res_path))
    etime = time.time()
...
if __name__ == '__main__':
  ...
  timeout = sys.argv[6]
  dev_id = sys.argv[7]
  main(pkg, arch, comp, pie, opt, timeout, dev_id)
